...
METHODS = [
    ...
    ("full_replay", "Full Replay / Large-native", (30, 210, 80)),
]
...
function show(){const r=records[+slider.value],a=aSel.value,b=bSel.value;document.querySelector('#frame-title').textContent=`Frame ${r.frame}`;document.querySelector('#gt').src=r.images.gt;document.querySelector('#native').src=r.images.large_native;document.querySelector('#a').src=r.images[a];document.querySelector('#b').src=r.images[b];document.querySelector('#scores').textContent=`Large-native ${fmt(r.scores.full_replay)} | ${methods[a]} ${fmt(r.scores[a])} | ${methods[b]} ${fmt(r.scores[b])}`}
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="cmmt-baseline-video-") as temp:
        ...
        for frame in range(start_frame, end_frame + 1):
            ...
            native_name = f"large_native_{stem}.jpg"
            native_panel = _panel(
            ...
            images["large_native"] = f"frames/{native_name}"
            for method_id, (label, color) in method_meta.items():
                if method_id == "full_replay":
                    images[method_id] = images["large_native"]
                    continue
