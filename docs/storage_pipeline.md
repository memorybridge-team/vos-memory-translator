...
GitHub (코드·설정)
        │ pull
        ▼
RunPod /workspace hot working set ── GPU inference/training
        │                               │
        │ local metric/validation       ▼
        └────────────────────────── complete run bundle
                                        │ 비동기/실험 종료 후
                                        ▼
                            외부 cold object storage
```

## 계층별 역할

### Hot — RunPod `/workspace/CMMT`
...
GPU가 읽는 파일은 모두 실험 시작 전에 이 계층에 있어야 한다. Object storage나
lazy mount에서 수천 개 JPEG/PNG를 학습 loop 중 직접 읽지 않는다.
